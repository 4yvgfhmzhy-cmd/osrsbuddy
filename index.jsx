...
function useXpTracker(skills) {
  ...
  return { gains, resetBaseline, baselineRef };
}

function XpTrackerTab({ skills, gains, resetBaseline, baselineRef }) {
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <div style={{ fontSize: 13, color: "var(--muted)" }}>
          {entries.length === 0 ? "No XP gains recorded yet — train a skill then hit ↻ Refresh in the header." : `${entries.length} skill${entries.length !== 1 ? "s" : ""} trained · ${fmt(totalGained)} total XP`}
        </div>
...
// ─────────────────────────────────────────────
// FEATURE 3 — BANK TAB (sprite grid, ingame style)
// ─────────────────────────────────────────────
const DEFAULT_BANK = [
...
// OSRS Wiki URL — item name with spaces → underscores
function wikiUrl(name) {
  return `https://oldschool.runescape.wiki/w/${encodeURIComponent(name.replace(/ /g, "_"))}`;
}

// Jagex official sprite CDN — works by item ID, no auth needed
function spriteUrl(id) {
  return `https://services.runescape.com/m=itemdb_oldschool/obj_sprite.gif?id=${id}`;
}

// Format qty exactly like OSRS: 100k = "100K" green, 10m = "10M" green, else white
function fmtQty(n) {
  if (n >= 10_000_000) return { text: (n / 1_000_000).toFixed(0) + "M", color: "#00ff80" };
  if (n >= 100_000)    return { text: (n / 1_000).toFixed(0) + "K",     color: "#00ff80" };
  if (n >= 1_000)      return { text: (n / 1_000).toFixed(1) + "K",     color: "#ffff00" };
  return { text: String(n), color: "#ffffff" };
}
...
function useBankItems() {
  ...
  function addItem(item) {
    ...
    if (items.find(i => i.id === item.id)) {
      save(items.map(i => i.id === item.id ? { ...i, qty: i.qty + item.qty } : i));
    } else {
      save([...items, item]);
    }
  }
...
// Inline styles for the bank UI
const bankUiStyles = `
  .bank-ui { background: #3d2b1a; border: 3px solid #8b6914; border-radius: 6px; padding: 0; overflow: hidden; }
  .bank-titlebar { background: linear-gradient(180deg, #6b4c1e 0%, #4a3210 100%); border-bottom: 2px solid #8b6914; padding: 6px 12px; display: flex; justify-content: space-between; align-items: center; }
  .bank-titlebar-text { font-family: 'Cinzel', serif; font-size: 14px; color: #f0d060; letter-spacing: 1px; }
  .bank-tab-row { background: #2a1f0e; border-bottom: 2px solid #8b6914; display: flex; align-items: center; gap: 2px; padding: 4px 8px; flex-wrap: wrap; }
  .bank-tab-btn { background: #3d2b1a; border: 1px solid #6b4c1e; border-radius: 3px 3px 0 0; padding: 3px 10px; font-size: 11px; color: #c8a84b; cursor: pointer; font-family: 'Inter', sans-serif; }
  .bank-tab-btn.selected { background: #5a3e1e; border-bottom-color: #5a3e1e; color: #f0d060; }
  .bank-slots-area { padding: 10px; background: #2a1c0a; min-height: 200px; }
  .bank-slots-grid { display: grid; grid-template-columns: repeat(auto-fill, 50px); gap: 4px; }
  .bank-slot { width: 50px; height: 50px; background: #1a1005; border: 1px solid #4a3515; border-radius: 2px; position: relative; cursor: pointer; user-select: none; display: flex; align-items: center; justify-content: center; transition: border-color 0.15s; flex-shrink: 0; }
  .bank-slot:hover { border-color: #c8a84b; background: #2a1c08; }
  .bank-slot a { display: block; position: absolute; inset: 0; cursor: pointer; }
  .bank-slot:focus-within { border-color: #f0d060; outline: none; }
  .btn:focus-visible, .nav-btn:focus-visible, .send-btn:focus-visible { outline: 2px solid var(--gold); outline-offset: 2px; }

  .bank-slot.selected-slot { border-color: #f0d060; background: #2e2208; }
  .bank-slot img { width: 36px; height: 36px; object-fit: contain; image-rendering: pixelated; }
  .bank-slot .qty-label { position: absolute; top: 1px; left: 2px; font-size: 9px; font-weight: 700; line-height: 1; text-shadow: 1px 1px 0 #000, -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000; font-family: 'Inter', sans-serif; pointer-events: none; }
  .bank-slot .img-error { font-size: 18px; }
  .bank-slot-empty { width: 50px; height: 50px; background: #111005; border: 1px dashed #3a2a10; border-radius: 2px; display: flex; align-items: center; justify-content: center; cursor: pointer; opacity: 0.5; transition: opacity 0.15s; }
  .bank-slot-empty:hover { opacity: 1; border-color: #c8a84b; }
  .bank-detail-bar { background: #1e1508; border-top: 2px solid #8b6914; padding: 10px 14px; display: flex; align-items: center; gap: 14px; flex-wrap: wrap; min-height: 54px; }
  .bank-detail-img { width: 40px; height: 40px; object-fit: contain; image-rendering: pixelated; flex-shrink: 0; }
  .bank-detail-info { flex: 1; }
  .bank-detail-name { font-size: 13px; color: #f0d060; font-family: 'Cinzel', serif; }
  .bank-detail-val  { font-size: 11px; color: var(--muted); margin-top: 2px; }
  .bank-detail-actions { display: flex; align-items: center; gap: 6px; }
  .bank-qty-input { width: 70px; background: #1a1005; border: 1px solid #6b4c1e; border-radius: 4px; color: #f0d060; font-size: 13px; text-align: center; padding: 4px 6px; font-family: 'Inter', sans-serif; }
  .bank-add-form { background: #1e1508; border-top: 2px solid #8b6914; padding: 10px 14px; display: flex; gap: 8px; flex-wrap: wrap; align-items: flex-end; }
`;

function BankSlot({ item, selected, onClick, onImgError, imgErrors }) {
  const q = fmtQty(item.qty);
  const errored = imgErrors[item.id];
  const sk = SKILLS.find(s => s.name === item.name);
  return (
    <div className={`bank-slot ${selected ? "selected-slot" : ""}`} onClick={onClick} title={`${item.name} × ${item.qty.toLocaleString()} — click to select, right-click to open wiki`}>
      <a
      ...
      />
      {errored
        ? <span className="img-error">{sk?.icon || "📦"}</span>
        : <img src={spriteUrl(item.id)} alt={item.name} onError={onImgError} loading="lazy" />
      }
      {item.qty > 1 && <span className="qty-label" style={{ color: q.color }}>{q.text}</span>}
    </div>
  );
}
...
function BankTabV2() {
  ...
  useEffect(() => {
    if (selectedItem) setEditQty(String(selectedItem.qty));
  }, [selected, selectedItem?.qty]);
  ...
  const handleQtyCommit = useCallback(() => {
    const v = parseInt(editQty);
    if (!isNaN(v) && v > 0) updateQty(selected, v);
    else setEditQty(String(selectedItem?.qty || 1));
  }, [editQty, selected, selectedItem, updateQty]);

  const handleAddItem = useCallback(() => {
    if (!newName.trim() || !newId.trim()) return;
    addItem({ name: newName.trim(), qty: Math.max(1, newQty), id: parseInt(newId) });
    setNewName(""); setNewQty(1); setNewId(""); setAdding(false);
  }, [newName, newId, newQty, addItem]);

  // Empty slots to pad grid to nearest row of 8
  const cols = 8;
  const emptySlots = (cols - (items.length % cols)) % cols;

  return (
    <div>
      <style>{bankUiStyles}</style>

      {/* Total value header */}
      <div className="bank-header" style={{ marginBottom: 12 }}>
      ...
      <div className="bank-ui">
        {/* Title bar */}
        <div className="bank-titlebar">
          <span className="bank-titlebar-text">🏦 Bank of RuneScape</span>
          <span style={{ fontSize: 11, color: "#c8a84b" }}>{items.length} item{items.length !== 1 ? "s" : ""}</span>
        </div>

        {/* Tab row */}
        <div className="bank-tab-row">
          <span className="bank-tab-btn selected">All items</span>
          <button className="btn" style={{ marginLeft: "auto", fontSize: 11 }} onClick={() => setAdding(v => !v)}>
            {adding ? "✕ Cancel" : "+ Add item"}
          </button>
        </div>

        {/* Slots grid */}
        <div className="bank-slots-area">
          <div className="bank-slots-grid">
            {items.map(item => (
              <BankSlot
                key={item.id}
                item={item}
                ...
                imgErrors={imgErrors}
                onImgError={() => setImgErrors(prev => ({ ...prev, [item.id]: true }))}
              />
            ))}
            {/* Empty slot padding */}
            {Array.from({ length: emptySlots }).map((_, i) => (
              <div key={`empty-${i}`} className="bank-slot-empty" onClick={() => setAdding(true)} title="Add item">
                <span style={{ fontSize: 18, color: "#4a3515" }}>+</span>
              </div>
            ))}
          </div>
        </div>

        {/* Detail bar — shown when an item is selected */}
        {selectedItem && (
          <div className="bank-detail-bar">
            {!imgErrors[selectedItem.id]
              ? <img className="bank-detail-img" src={spriteUrl(selectedItem.id)} alt={selectedItem.name} />
              : <span style={{ fontSize: 28 }}>{SKILLS.find(s => s.name === selectedItem.name)?.icon || "📦"}</span>
            }
            <div className="bank-detail-info">
              <a
                href={wikiUrl(selectedItem.name)}
                target="_blank"
                rel="noopener noreferrer"
                style={{ textDecoration: "none" }}
                title={`Open ${selectedItem.name} on OSRS Wiki`}
              >
                <div className="bank-detail-name" style={{ cursor: "pointer", textDecoration: "underline", textDecorationColor: "rgba(240,208,96,0.4)", textUnderlineOffset: 3 }}>
                  {selectedItem.name} ↗
                </div>
              </a>
              <div className="bank-detail-val">
                {prices[selectedItem.id] ? `${fmt(prices[selectedItem.id] * selectedItem.qty)} gp · ${prices[selectedItem.id].toLocaleString()} ea` : "Price unavailable"}
              </div>
            </div>
            <div className="bank-detail-actions">
              <button onClick={() => updateQty(selectedItem.id, selectedItem.qty - 1)} aria-label="Decrease quantity" style={{ background: "#3d2b1a", border: "1px solid #6b4c1e", color: "#f0d060", borderRadius: 4, width: 24, height: 24, cursor: "pointer", fontSize: 16, lineHeight: 1 }}>−</button>
              <input
                className="bank-qty-input"
                type="number"
                min="1"
                value={editQty}
              ...
              />
              <button onClick={() => updateQty(selectedItem.id, selectedItem.qty + 1)} aria-label="Increase quantity" style={{ background: "#3d2b1a", border: "1px solid #6b4c1e", color: "#f0d060", borderRadius: 4, width: 24, height: 24, cursor: "pointer", fontSize: 16, lineHeight: 1 }}>+</button>
              <button onClick={() => { removeItem(selectedItem.id); setSelected(null); }} style={{ background: "#4a1a1a", border: "1px solid #8b1a1a", color: "#ff6b6b", borderRadius: 4, padding: "3px 10px", cursor: "pointer", fontSize: 11, marginLeft: 4 }}>Remove</button>
            </div>
          </div>
        )}
        ...
        {adding && (
          <div className="bank-add-form">
            <div><label className="input-label">Item Name</label>
              <input className="splash-input" style={{ width: 150, marginBottom: 0 }} placeholder="e.g. Dragon bones" value={newName} onChange={e => setNewName(e.target.value)} /></div>
            <div><label className="input-label">Item ID</label>
              <input className="splash-input" style={{ width: 90, marginBottom: 0 }} placeholder="e.g. 536" value={newId} onChange={e => setNewId(e.target.value)} /></div>
            <div><label className="input-label">Quantity</label>
              <input className="splash-input" style={{ width: 80, marginBottom: 0 }} type="number" min="1" value={newQty} onChange={e => setNewQty(parseInt(e.target.value) || 1)} /></div>
            <button className="send-btn" style={{ alignSelf: "flex-end" }} onClick={handleAddItem}>Add</button>
            <div style={{ fontSize: 10, color: "var(--muted)", alignSelf: "flex-end", paddingBottom: 4 }}>
              Find item IDs at <span style={{ color: "var(--gold)" }}>prices.runescape.wiki</span>
            </div>
          </div>
        )}
      </div>
...
const styles = skeletonStyles + `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { margin: 0; background: #1a1209; scroll-behavior: smooth; }
...
function Splash({ onEnter }) {
  ...
  const [apiKey, setApiKey] = useState(() => localStorage.getItem("osrs_api_key") || "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");
  ...
  async function handleEnter() {
    ...
    try {
      ...
      if (apiKey.trim()) localStorage.setItem("osrs_api_key", apiKey.trim());
      onEnter(saved);
  ...
  return (
    <div className="splash">
      ...
      <div className="lb-bg">
        {LB_ROWS.map((r, i) => (
          <div key={r} className="lb-row" style={{ animationDelay: `${i * 0.12}s` }}>
            {i + 1}. {r}
          </div>
        ))}
      </div>
...
function ChatTab({ skills, username }) {
  ...
  async function send(text) {
    ...
    const apiKey = localStorage.getItem("osrs_api_key") || "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";
    if (!apiKey) {
      setMessages([...newMsgs, { role: "ai", text: "⚠️ No API key set. Enter your OpenRouter API key below and try again." }]);
...
Give practical, specific OSRS advice based on THEIR actual stats. Be concise but helpful. Reference their actual levels when relevant. Use OSRS terminology naturally. If they ask about XP needed, calculate it from their current XP. Always suggest the most efficient next step.`;
  ...
  return (
    <div className="chat-container">
      {showKeyInput && (
        <div style={{ background: "var(--parch2)", border: "1px solid var(--gold)", borderRadius: 8, padding: "10px 14px", marginBottom: 10, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <span style={{ fontSize: 12, color: "var(--gold)", flex: "0 0 auto" }}>🔑 OpenRouter API Key</span>
          <input
            ...
            onKeyDown={e => { if (e.key === "Enter" && keyDraft.trim()) { localStorage.setItem("osrs_api_key", keyDraft.trim()); setShowKeyInput(false); setKeyDraft(""); }}}
            style={{ flex: 1, minWidth: 200 }}
          />
          <button className="send-btn" onClick={() => { if (keyDraft.trim()) { localStorage.setItem("osrs_api_key", keyDraft.trim()); setShowKeyInput(false); setKeyDraft(""); }}}>Save</button>
          {localStorage.getItem("osrs_api_key") && (
...
export default function OSRSTracker() {
  ...
  const { gains, resetBaseline, baselineRef } = useXpTracker(skills);
  ...
  return (
    <div className="tracker">
      ...
      <nav className="nav">
        {tabs.map(t => <button key={t} title={t} className={`nav-btn ${tab === t ? "active" : ""}`} onClick={() => setTab(t)}>{t}</button>)}
      </nav>
      <div className="content">
        {tab === "Skills"  && (syncStatus === "offline" && Object.values(skills).every(s => s.xp === 0) ? <SkeletonSkills /> : <SkillsTab skills={skills} />)}
        {tab === "Gains"   && <XpTrackerTab skills={skills} gains={gains} resetBaseline={resetBaseline} baselineRef={baselineRef} />}
